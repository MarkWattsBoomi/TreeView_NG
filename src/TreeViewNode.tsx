...
import { _TreeView, eDebugLevel } from './TreeView';
...
export default class TreeViewNode extends React.Component<any, any> {
    ...
    componentDidMount() {
        const root: _TreeView = this.props.root;
    ...
    showContextMenu(e: any) {
        ...
        if (this.contextMenu && node.isEnabled()) {
            Object.keys(root.component.outcomes).forEach((key: string) => {
                const outcome: FlowOutcome = root.component.outcomes[key];
    ...
    showInfo() {
        ...
        const content: any = (
            <ItemInfo
                item={node}
                display={root.component.columns}
            />
        );
    ...
    render() {
        ...
        let nodeIcon: any;
        if (root.component.getAttribute('ShowInfo', 'false').toLowerCase() === 'true') {
            nodeIcon = (
                <span
